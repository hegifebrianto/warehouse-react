...
import { ICreateProductDto, ProductDto } from "../../common/types";
...
function ProductCreateForm() {
  ...
  function onSubmit(inputs: Inputs) {
    ...
    axios
      ...
      .then((res) => {
        navigate(
          Utils.dashboardUrl(appContext) + `/products/view/${res.data.id}`
        );
  ...
  return (
    <Form onSubmit={handleSubmit(onSubmit)} loading={loading}>
      <FormField label="Organization">
        <FromInput value={appContext.organization.name} readOnly />
      </FormField>

      <FormField label="Product name" errors={errors.name} required>
        <FromInput
      ...
      <FormField label="Product description" errors={errors.description}>
        <FormTextArea
          {...register("description", { setValueAs: FieldTransformers.string })}
        />
      </FormField>
      ...
      <FormField label="Unit" errors={errors.unit}>
        <FromInput
